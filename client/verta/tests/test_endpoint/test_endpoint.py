...
class TestEndpoint:
    ...
    def test_get_by_name(self, client, created_entities):
        ...
        dummy_endpoint = client.set_endpoint(
            path2
    ...
    def test_download_manifest(self, client, in_tempdir):
        ...
        resources = Resources(cpu=0.1, memory="128Mi")
        ...
        containers = manifest["spec"]["function"]["spec"]["templates"]["deployment"][
            "spec"
        ]["template"]["spec"]["containers"]
        retrieved_env_vars = {
            env_var["name"]: env_var["value"] for env_var in containers[0]["env"]
        }
        assert retrieved_env_vars == env_vars

    def test_direct_update(
        self, client, created_entities, experiment_run, model_for_deployment
    ):
        experiment_run.log_model(model_for_deployment["model"], custom_modules=[])
        experiment_run.log_environment(Python(["scikit-learn"]))
        ...
        assert (
            len(new_build_ids) - len(new_build_ids.intersection(original_build_ids)) > 0
        )
    ...
    def test_update_wait(
        ...
        with pytest.raises(RuntimeError) as excinfo:
            endpoint.update(
                model_version, DirectUpdateStrategy(), True
            )  # this should fail, and not take forever!

        excinfo_value = str(excinfo.value).strip()
        assert (
            "Could not find a version that satisfies the requirement blahblahblah==3.6.0"
            in excinfo_value
        )
    ...
    def test_update_from_json_config(
        self, client, in_tempdir, created_entities, experiment_run, model_for_deployment
    ):
                ...
                "rules": [
                    {
                        ...
                        "rule_parameters": [{"name": "threshold", "value": "0.1"}],
                    },
                    {
                        "rule": "error_4xx_rate",
                        "rule_parameters": [{"name": "threshold", "value": "1"}],
                    },
                ],
            },
        }
    ...
    def test_update_from_yaml_config(
        self, client, in_tempdir, created_entities, experiment_run, model_for_deployment
    ):
    ...
    def test_update_with_parameters(
        ...
        strategy.add_rule(MaximumAverageLatencyThresholdRule(0.8))
        updated_status = endpoint.update(
            experiment_run,
            strategy,
    ...
    def test_create_update_body(self):
        ...
        parameter_json = endpoint._create_update_body(
            DirectUpdateStrategy(), resources, None, env_vars
        )
    ...
    def test_get_deployed_model(
        ...
        assert (
            '-H "Access-token: {}"'.format(token).lower() in deployed_model_curl.lower()
        )
        ...
        endpoint.get_deployed_model().predict(
            [x]
        )  # should succeed, because the endpoint can still service requests
    ...
    def test_update_from_model_version(self, client, model_version, created_entities):
        ...
        assert np.array_equal(
            endpoint.get_deployed_model().predict(test_data),
            classifier.predict(test_data),
        )

    def test_update_from_json_config_model_version(
        self, client, in_tempdir, created_entities, model_version
    ):
        np = pytest.importorskip("numpy")
        json = pytest.importorskip("json")
        sklearn = pytest.importorskip("sklearn")
    ...
    def test_update_autoscaling(
        ...
        autoscaling = Autoscaling(
            min_replicas=1, max_replicas=2, min_scale=0.5, max_scale=2.0
        )
    ...
    def test_update_with_custom_module(self, client, model_version, created_entities):
        ...
        with sys_path_manager() as sys_path:
            ...
            model_version.log_model(
                classifier, custom_modules=["models/"], model_api=model_api
            )
            ...
            prediction = torch.tensor(
                endpoint.get_deployed_model().predict(test_data.tolist())
            )
            assert torch.all(classifier(test_data).eq(prediction))

    def test_update_from_json_config_with_params(
        self, client, in_tempdir, created_entities, experiment_run, model_for_deployment
    ):
        yaml = pytest.importorskip("yaml")
        experiment_run.log_model(model_for_deployment["model"], custom_modules=[])
                ...
                "metrics": [
                    {
                        ...
                        "parameters": [{"name": "target", "value": "0.5"}],
                    },
                    {
                        "metric": "memory_utilization",
                        "parameters": [{"name": "target", "value": "0.7"}],
                    },
                ],
            },
        ...
        )._as_dict()  # config is `cpu`, wire is `cpu_millis`
        assert (
            endpoint.get_update_status()["update_request"]["resources"]
            == resources_dict
        )
    ...
    def test_update_from_version_diff_workspace_no_access_error(
        self, client_2, model_version, created_entities
    ):
        np = pytest.importorskip("numpy")
        sklearn = pytest.importorskip("sklearn")
        from sklearn.linear_model import LogisticRegression

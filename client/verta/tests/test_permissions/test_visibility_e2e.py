...
class TestAccess:
    ...
    def test_repository(self, client, client_2, organization, created_entities):
        ...
        # private
        private_repo = client.set_repository(
            _utils.generate_default_name(), visibility=Private()
        )
        ...
        # read-only
        read_repo = client.set_repository(
            _utils.generate_default_name(), visibility=OrgCustom(write=False)
        )
        ...
        # read-write
        write_repo = client.set_repository(
            _utils.generate_default_name(), visibility=OrgCustom(write=True)
        )
        try:

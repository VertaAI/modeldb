...
class TestLogModel:
    ...
    def test_model_class(self, deployable_entity, model_for_deployment):
        ...
        retrieved_model_api = ModelAPI.from_file(
...
class TestDeployability:
    ...
    def test_log_environment(self, deployable_entity):
        ...
        new_env = Python([])
        with pytest.raises(ValueError):
            deployable_entity.log_environment(new_env)
        deployable_entity.log_environment(new_env, overwrite=True)
        assert str(new_env) == str(deployable_entity.get_environment())
    ...
    def test_download_docker_context(
        self, deployable_entity, model_for_deployment, in_tempdir
    ):

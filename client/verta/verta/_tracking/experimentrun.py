...
class ExperimentRun(_DeployableEntity):
    ...
    def _log_artifact(
        self, key, artifact, artifact_type, extension=None, method=None, overwrite=False
    ):
        """
    ...
    def _log_artifact_path(self, key, artifact_path, artifact_type, overwrite=False):
        ...
        Message = _ExperimentRunService.LogArtifact
        artifact_msg = _CommonCommonService.Artifact(
            key=key, path=artifact_path, path_only=True, artifact_type=artifact_type
        )
        msg = Message(id=self.id, artifact=artifact_msg)
        data = _utils.proto_to_json(msg)
        if overwrite:
    ...
    def _get_artifact(self, key):
        ...
        artifact = {artifact.key: artifact for artifact in response_msg.artifacts}.get(
            key
        )
        if artifact is None:
    ...
    def log_model_for_deployment(
        self, model, model_api, requirements, train_features=None, train_targets=None
    ):
        """
        ...
        if train_data is not None:
            self._log_artifact(
                "train_data",
                train_data,
                _CommonCommonService.ArtifactTypeEnum.DATA,
                "csv",
            )
    ...
    def log_requirements(self, requirements, overwrite=False):
        ...
        if isinstance(requirements, six.string_types):
            ...
            raise TypeError(
                "`requirements` must be either str or list of str, not {}".format(
                    type(requirements)
                )
            )
    ...
    def get_deployment_status(self):
        ...
        if "api" in status:
            status.update(
                {
                    "url": "{}://{}{}".format(
                        self._conn.scheme, self._conn.socket, status.pop("api")
                    )
                }
            )
    ...
    def deploy(self, path=None, token=None, no_token=False, wait=False):
        ...
        try:
            ...
            if response.status_code == 400:
                ...
                six.raise_from(
                    RuntimeError("unable to deploy due to {}".format(error_text)), None
                )
            else:
                raise e
    ...
    def download_deployment_yaml(
        self, download_to_path, path=None, token=None, no_token=False
    ):
        """
    ...
    def get_commit(self):
        ...
        key_paths = {
            ...
            for key, location_msg in response_msg.versioned_inputs.key_location_map.items()
        }

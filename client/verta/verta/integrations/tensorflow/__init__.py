...
from tensorflow.core.framework.summary_pb2 import (
    Summary,  # pylint: disable=import-error, no-name-in-module
)
from tensorflow.core.util.event_pb2 import (
    Event,  # pylint: disable=import-error, no-name-in-module
)
    ...
    for event in events:
        ...
        if event.WhichOneof("what") == "summary":
            for value in event.summary.value:
                ...
                elif value.WhichOneof("value") == "tensor" and tf.executing_eagerly():
                    np_value = tf.make_ndarray(value.tensor)
                    if (
                        np_value.size == 1  # scalar tensors can be logged
...
def log_tensorboard_events(run, log_dir):
    ...
    for subdir in ("train", "validation"):  # TensorFlow 2.X puts events in subdirs
        if subdir in os.listdir(log_dir) and os.path.isdir(
            os.path.join(log_dir, subdir)
        ):
            for events_filepath in _get_events_filepaths(os.path.join(log_dir, subdir)):
                event_observations = _collect_observations(
                    events_filepath, prefix=subdir
                )
                observations.extend(event_observations)
    ...
    _utils.make_request(
        "POST",
        "{}://{}/api/v1/modeldb/experiment-run/logObservations".format(
            run._conn.scheme, run._conn.socket
        ),
        run._conn,
        json={
            "id": run.id,

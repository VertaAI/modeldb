...
case class ModeldbLogObservationResponse (
  ...
  def toJson(): JValue = ModeldbLogObservationResponse.toJson(this)
}

object ModeldbLogObservationResponse {
  def toJson(obj: ModeldbLogObservationResponse): JObject = {
    new JObject(
      List[Option[JField]](
  ...
  def fromJson(value: JValue): ModeldbLogObservationResponse =
    value match {
      case JObject(fields) => {

...
case class UacAddUserResponse (
  ...
  def toJson(): JValue = UacAddUserResponse.toJson(this)
}

object UacAddUserResponse {
  def toJson(obj: UacAddUserResponse): JObject = {
    new JObject(
      List[Option[JField]](
  ...
  def fromJson(value: JValue): UacAddUserResponse =
    value match {
      case JObject(fields) => {

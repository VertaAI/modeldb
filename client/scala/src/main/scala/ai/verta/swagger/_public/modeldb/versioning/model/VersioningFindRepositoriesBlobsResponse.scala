...
object VersioningFindRepositoriesBlobsResponse {
  def toJson(obj: VersioningFindRepositoriesBlobsResponse): JObject = {
    new JObject(
      List[Option[JField]](
  ...
  def fromJson(value: JValue): VersioningFindRepositoriesBlobsResponse =
    value match {
      case JObject(fields) => {

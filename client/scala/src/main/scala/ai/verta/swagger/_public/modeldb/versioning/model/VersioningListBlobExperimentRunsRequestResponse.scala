...
case class VersioningListBlobExperimentRunsRequestResponse (
  runs: Option[List[ModeldbExperimentRun]] = None,
  total_records: Option[BigInt] = None
) extends BaseSwagger {
  def toJson(): JValue = VersioningListBlobExperimentRunsRequestResponse.toJson(this)
}

object VersioningListBlobExperimentRunsRequestResponse {
  def toJson(obj: VersioningListBlobExperimentRunsRequestResponse): JObject = {
    new JObject(
      List[Option[JField]](
  ...
  def fromJson(value: JValue): VersioningListBlobExperimentRunsRequestResponse =
    value match {
      case JObject(fields) => {

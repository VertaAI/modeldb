...
case class ModeldbLogObservationsResponse (
  ...
  def toJson(): JValue = ModeldbLogObservationsResponse.toJson(this)
}

object ModeldbLogObservationsResponse {
  def toJson(obj: ModeldbLogObservationsResponse): JObject = {
    new JObject(
      List[Option[JField]](
  ...
  def fromJson(value: JValue): ModeldbLogObservationsResponse =
    value match {
      case JObject(fields) => {

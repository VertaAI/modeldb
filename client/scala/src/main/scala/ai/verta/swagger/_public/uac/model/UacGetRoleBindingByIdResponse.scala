...
case class UacGetRoleBindingByIdResponse (
  ...
  def toJson(): JValue = UacGetRoleBindingByIdResponse.toJson(this)
}

object UacGetRoleBindingByIdResponse {
  def toJson(obj: UacGetRoleBindingByIdResponse): JObject = {
    new JObject(
      List[Option[JField]](
  ...
  def fromJson(value: JValue): UacGetRoleBindingByIdResponse =
    value match {
      case JObject(fields) => {

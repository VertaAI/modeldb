...
case class ModeldbDeleteArtifactResponse (
  ...
  def toJson(): JValue = ModeldbDeleteArtifactResponse.toJson(this)
}

object ModeldbDeleteArtifactResponse {
  def toJson(obj: ModeldbDeleteArtifactResponse): JObject = {
    new JObject(
      List[Option[JField]](
  ...
  def fromJson(value: JValue): ModeldbDeleteArtifactResponse =
    value match {
      case JObject(fields) => {

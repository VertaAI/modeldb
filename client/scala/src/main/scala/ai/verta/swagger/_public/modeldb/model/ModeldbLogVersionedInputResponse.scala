...
case class ModeldbLogVersionedInputResponse (
  experiment_run: Option[ModeldbExperimentRun] = None
) extends BaseSwagger {
  def toJson(): JValue = ModeldbLogVersionedInputResponse.toJson(this)
}

object ModeldbLogVersionedInputResponse {
  def toJson(obj: ModeldbLogVersionedInputResponse): JObject = {
    new JObject(
      List[Option[JField]](
  ...
  def fromJson(value: JValue): ModeldbLogVersionedInputResponse =
    value match {
      case JObject(fields) => {

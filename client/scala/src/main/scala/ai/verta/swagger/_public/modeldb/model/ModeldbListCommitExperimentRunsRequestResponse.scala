...
case class ModeldbListCommitExperimentRunsRequestResponse (
  runs: Option[List[ModeldbExperimentRun]] = None,
  total_records: Option[BigInt] = None
) extends BaseSwagger {
  def toJson(): JValue = ModeldbListCommitExperimentRunsRequestResponse.toJson(this)
}

object ModeldbListCommitExperimentRunsRequestResponse {
  def toJson(obj: ModeldbListCommitExperimentRunsRequestResponse): JObject = {
    new JObject(
      List[Option[JField]](
  ...
  def fromJson(value: JValue): ModeldbListCommitExperimentRunsRequestResponse =
    value match {
      case JObject(fields) => {

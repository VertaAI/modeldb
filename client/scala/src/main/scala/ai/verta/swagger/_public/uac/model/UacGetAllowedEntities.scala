...
case class UacGetAllowedEntities (
  ...
  def toJson(): JValue = UacGetAllowedEntities.toJson(this)
}

object UacGetAllowedEntities {
  def toJson(obj: UacGetAllowedEntities): JObject = {
    new JObject(
      List[Option[JField]](
  ...
  def fromJson(value: JValue): UacGetAllowedEntities =
    value match {
      case JObject(fields) => {

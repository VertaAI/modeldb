...
class MetadataServiceApi(client: HttpClient, val basePath: String = "/v1") {
  ...
  def GetLabelsAsync(id_id_type: String, id_int_id: BigInt, id_string_id: String)(implicit ec: ExecutionContext): Future[Try[MetadataGetLabelsRequestResponse]] = {
    val __query = Map[String,String](
  ...
  def GetLabels(id_id_type: String, id_int_id: BigInt, id_string_id: String)(implicit ec: ExecutionContext): Try[MetadataGetLabelsRequestResponse] = Await.result(GetLabelsAsync(id_id_type, id_int_id, id_string_id), Duration.Inf)

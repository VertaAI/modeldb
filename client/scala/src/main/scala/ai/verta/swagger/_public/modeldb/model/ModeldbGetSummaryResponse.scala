...
case class ModeldbGetSummaryResponse (
  ...
  total_experiment: Option[] = None,
  total_experiment_runs: Option[] = None
) extends BaseSwagger {
  ...
  def toJson(obj: ModeldbGetSummaryResponse): JObject = {
    new JObject(
      List[Option[JField]](
        ...
        obj.total_experiment.map(x => JField("total_experiment", (x))),
        obj.total_experiment_runs.map(x => JField("total_experiment_runs", (x)))
      ).flatMap(x => x match {
        case Some(y) => List(y)
        case None => Nil
      })
    ...
    value match {
      case JObject(fields) => {
        ...
        ModeldbGetSummaryResponse(
          ...
          total_experiment = fieldsMap.get("total_experiment").map(),
          total_experiment_runs = fieldsMap.get("total_experiment_runs").map()
        )
      }

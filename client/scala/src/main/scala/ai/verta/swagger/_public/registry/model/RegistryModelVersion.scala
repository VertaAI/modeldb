...
case class RegistryModelVersion (
  ...
  assets: Option[List[CommonArtifact]] = None,
  ...
  def toJson(obj: RegistryModelVersion): JObject = {
    new JObject(
      List[Option[JField]](
        ...
        obj.assets.map(x => JField("assets", ((x: List[CommonArtifact]) => JArray(x.map(((x: CommonArtifact) => CommonArtifact.toJson(x)))))(x))),
    ...
    value match {
      case JObject(fields) => {
        ...
        RegistryModelVersion(
          ...
          assets = fieldsMap.get("assets").map((x: JValue) => x match {case JArray(elements) => elements.map(CommonArtifact.fromJson); case _ => throw new IllegalArgumentException(s"unknown type ${x.getClass.toString}")}),

...
case class ModeldbDeleteExperiments (
  ...
  def toJson(): JValue = ModeldbDeleteExperiments.toJson(this)
}

object ModeldbDeleteExperiments {
  def toJson(obj: ModeldbDeleteExperiments): JObject = {
    new JObject(
      List[Option[JField]](
  ...
  def fromJson(value: JValue): ModeldbDeleteExperiments =
    value match {
      case JObject(fields) => {

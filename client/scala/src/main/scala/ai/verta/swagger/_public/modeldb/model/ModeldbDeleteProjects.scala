...
case class ModeldbDeleteProjects (
  ...
  def toJson(): JValue = ModeldbDeleteProjects.toJson(this)
}

object ModeldbDeleteProjects {
  def toJson(obj: ModeldbDeleteProjects): JObject = {
    new JObject(
      List[Option[JField]](
  ...
  def fromJson(value: JValue): ModeldbDeleteProjects =
    value match {
      case JObject(fields) => {

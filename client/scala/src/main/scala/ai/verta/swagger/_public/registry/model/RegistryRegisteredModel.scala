...
case class RegistryRegisteredModel (
  ...
  parent_workspace_id: Option[BigInt] = None,
  ...
  def toJson(obj: RegistryRegisteredModel): JObject = {
    new JObject(
      List[Option[JField]](
        ...
        obj.parent_workspace_id.map(x => JField("parent_workspace_id", JInt(x))),
    ...
    value match {
      case JObject(fields) => {
        ...
        RegistryRegisteredModel(
          ...
          parent_workspace_id = fieldsMap.get("parent_workspace_id").map(JsonConverter.fromJsonInteger),

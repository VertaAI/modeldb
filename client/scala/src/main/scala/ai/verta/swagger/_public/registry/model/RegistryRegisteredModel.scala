...
case class RegistryRegisteredModel (
  ...
  legacy_workspace_id: Option[String] = None,
  ...
  def toJson(obj: RegistryRegisteredModel): JObject = {
    new JObject(
      List[Option[JField]](
        ...
        obj.legacy_workspace_id.map(x => JField("legacy_workspace_id", JString(x))),
    ...
    value match {
      case JObject(fields) => {
        ...
        RegistryRegisteredModel(
          ...
          legacy_workspace_id = fieldsMap.get("legacy_workspace_id").map(JsonConverter.fromJsonString),

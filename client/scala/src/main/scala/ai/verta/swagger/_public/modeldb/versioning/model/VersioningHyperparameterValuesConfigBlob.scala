...
case class VersioningHyperparameterValuesConfigBlob (
  int_value: Option[] = None,
  float_value: Option[] = None,
...
object VersioningHyperparameterValuesConfigBlob {
  def toJson(obj: VersioningHyperparameterValuesConfigBlob): JObject = {
    new JObject(
      List[Option[JField]](
        obj.int_value.map(x => JField("int_value", (x))),
        obj.float_value.map(x => JField("float_value", (x))),
    ...
    value match {
      case JObject(fields) => {
        ...
        VersioningHyperparameterValuesConfigBlob(
          ...
          int_value = fieldsMap.get("int_value").map(),
          float_value = fieldsMap.get("float_value").map(),

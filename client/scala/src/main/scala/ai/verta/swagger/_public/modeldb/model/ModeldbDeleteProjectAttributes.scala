...
case class ModeldbDeleteProjectAttributes (
  attribute_keys: Option[List[String]] = None,
  delete_all: Option[Boolean] = None,
  id: Option[String] = None
) extends BaseSwagger {
  def toJson(): JValue = ModeldbDeleteProjectAttributes.toJson(this)
}

object ModeldbDeleteProjectAttributes {
  def toJson(obj: ModeldbDeleteProjectAttributes): JObject = {
    new JObject(
      List[Option[JField]](
  ...
  def fromJson(value: JValue): ModeldbDeleteProjectAttributes =
    value match {
      case JObject(fields) => {

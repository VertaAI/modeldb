...
case class ModeldbDeleteExperimentRunAttributes (
  attribute_keys: Option[List[String]] = None,
  delete_all: Option[Boolean] = None,
  id: Option[String] = None
) extends BaseSwagger {
  def toJson(): JValue = ModeldbDeleteExperimentRunAttributes.toJson(this)
}

object ModeldbDeleteExperimentRunAttributes {
  def toJson(obj: ModeldbDeleteExperimentRunAttributes): JObject = {
    new JObject(
      List[Option[JField]](
  ...
  def fromJson(value: JValue): ModeldbDeleteExperimentRunAttributes =
    value match {
      case JObject(fields) => {

...
case class ModeldbLogAttributeResponse (
  ...
  def toJson(): JValue = ModeldbLogAttributeResponse.toJson(this)
}

object ModeldbLogAttributeResponse {
  def toJson(obj: ModeldbLogAttributeResponse): JObject = {
    new JObject(
      List[Option[JField]](
  ...
  def fromJson(value: JValue): ModeldbLogAttributeResponse =
    value match {
      case JObject(fields) => {

...
case class ModeldbLogDatasetResponse (
  ...
  def toJson(): JValue = ModeldbLogDatasetResponse.toJson(this)
}

object ModeldbLogDatasetResponse {
  def toJson(obj: ModeldbLogDatasetResponse): JObject = {
    new JObject(
      List[Option[JField]](
  ...
  def fromJson(value: JValue): ModeldbLogDatasetResponse =
    value match {
      case JObject(fields) => {

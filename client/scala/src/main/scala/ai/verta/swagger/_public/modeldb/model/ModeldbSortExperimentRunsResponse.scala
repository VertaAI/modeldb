...
case class ModeldbSortExperimentRunsResponse (
  ...
  def toJson(): JValue = ModeldbSortExperimentRunsResponse.toJson(this)
}

object ModeldbSortExperimentRunsResponse {
  def toJson(obj: ModeldbSortExperimentRunsResponse): JObject = {
    new JObject(
      List[Option[JField]](
  ...
  def fromJson(value: JValue): ModeldbSortExperimentRunsResponse =
    value match {
      case JObject(fields) => {

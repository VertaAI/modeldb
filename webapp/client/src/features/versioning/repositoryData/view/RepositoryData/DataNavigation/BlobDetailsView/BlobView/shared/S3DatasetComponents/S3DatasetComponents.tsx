...
import { IS3DatasetComponentBlob } from 'core/shared/models/Versioning/Blob/DatasetBlob';
...
interface ILocalProps {
  data: IS3DatasetComponentBlob[];
}

const S3DatasetComponents = (props: ILocalProps) => {
  const getRowKey = useCallback(
    (row: IS3DatasetComponentBlob) => row.sha256,
    []
  );
    ...
    <Table
      ...
      columnDefinitions={[
        ...
        },
        {
          ...
          type: 's3VersionId',
          render: row => (
            <TextWithCopyTooltip copyText={row.s3VersionId}>
              <span title={row.s3VersionId}>{row.s3VersionId}</span>
            </TextWithCopyTooltip>
          ),
          width: '17%',
          withSort: true,
          getValue: row => row.s3VersionId,
        },
        {
          title: 'MD5',
          type: 'md5',
...
export default S3DatasetComponents;

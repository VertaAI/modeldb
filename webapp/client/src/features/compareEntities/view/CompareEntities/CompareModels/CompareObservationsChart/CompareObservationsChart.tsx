...
import Legend, { LegendItems, IObservationLineDataWithColor } from 'shared/view/domain/ModelRecord/ModelRecordProps/Observations/Observations/ObservationsModelPage/ObservationsChart/Legend/Legend';
...
interface ILocalProps {
    modelsObservations: [IModelObservations, IModelObservations];
}

type IObservationSelectionByModel = Record<IModelObservations['number'], Record<string, boolean>>;

export const CompareObservartionsChart = ({ modelsObservations }: ILocalProps) => {
    const [selectedObservationsByModel, setSelectedObservations] = React.useState<IObservationSelectionByModel>(() => getInitialSelectionByModel({ modelsObservations }));
    const toggleObservationSelection = (modelNumber: number, attributeKey: string) => {
    ...
    const { xAxisType, xAxisTypeSelect } = useXAxisTypeSelect(modelsObservations.flatMap(({ observations }) => observations));

    const modelsGroupedObservations = modelsObservations.map(({ number, observations }) => ({ number, groupedObservations: groupObservationsByAttributeKey(observations) }));
    const chartData = modelsGroupedObservations
        .flatMap(({ number, groupedObservations }) =>
            getObservationLineData(groupedObservations, selectedObservationsByModel[number])
                .map((d) => ({ ...d, modelNumber: number }))
        );

    return modelsGroupedObservations.some(({ groupedObservations }) => Object.keys(groupedObservations).length > 0) ? (
        <PageCard>
            <div className={styles.root}>
                <PageHeader title="Observations chart" size="small" withoutSeparator={true} />
            </div>
            {xAxisTypeSelect}
            <div className={styles.observations_wrapper}>
                <div className={styles.observations}>
                    {modelsGroupedObservations.map(({ number, groupedObservations }) => (
                        <GroupedObservations
                            title={`Model ${number}`}
                            groupedObservations={groupedObservations}
                            onToggleObservationSelection={(attributeKey) => toggleObservationSelection(number, attributeKey)}
                            selectedObservations={selectedObservationsByModel[number]}
                        />
                    ))}
                </div>
                <div className={styles.observations_chart_container}>
                    <ObservationsChart
                        data={chartData}
                        xAxisType={xAxisType}
                        renderLegend={({ data }: { data: (IObservationLineDataWithColor & { modelNumber: number })[] }) => {
                            const modelsChartData = R.groupBy(({ modelNumber }) => String(modelNumber), data);
                            ...
                            return (
                                <Legend
                                    ...
                                    renderLegendItems={() => (
                                        <>
                                            {R.sortBy(([modelNumber]) => modelNumber, R.toPairs(modelsChartData))
...
const GroupedObservations = ({ groupedObservations, selectedObservations, title, onToggleObservationSelection }: {
    title: string;
...
function getInitialSelectionByModel({ modelsObservations }: { modelsObservations: IModelObservations[] }): IObservationSelectionByModel {
    ...
    return R.fromPairs(modelsObservations.map(({ number, observations }) => [number, getInitialSelection(observations)]));
}

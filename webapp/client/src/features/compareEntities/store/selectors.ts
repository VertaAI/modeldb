...
const getModelsDifferentProps = (
  ...
  return {
    codeVersion: {
      usual: compareCodeVersion(
        modelRecord1.codeVersion,
        modelRecord2.codeVersion
      ),
      fromBlobs: compareCodeVersionsFromBlobs(
        modelRecord1.codeVersionsFromBlob,
        modelRecord2.codeVersionsFromBlob
      ),
    },
    ownerId: modelRecord1.ownerId !== modelRecord2.ownerId,
...
const compareCodeVersionsFromBlobs = (
  codeVersionsFromBlob1?: ICodeVersionsFromBlob,
  codeVersionsFromBlob2?: ICodeVersionsFromBlob
): IModelsDifferentProps['codeVersion']['fromBlobs'] => {
  if (!codeVersionsFromBlob1 && !codeVersionsFromBlob2) {
    return null;
  }
  const blobLocations = [
    ...Object.keys(codeVersionsFromBlob1 || {}),
    ...Object.keys(codeVersionsFromBlob2 || {}),
  ];
  return R.fromPairs(
    blobLocations.map(blobLocation => [
      blobLocation,
      compareCodeVersion(
        (codeVersionsFromBlob1 || {})[blobLocation],
        (codeVersionsFromBlob2 || {})[blobLocation]
      ),
    ])
  );
};

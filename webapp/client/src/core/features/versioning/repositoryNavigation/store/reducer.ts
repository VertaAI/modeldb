...
const getHistoryMove = (action: LocationChangeAction<any>, history: RepositoryHistory) => {
    ...
    switch (action.type) {
        case '@@router/LOCATION_CHANGE': {
            if (isRepositoryPathname(action.payload.location.pathname)) {
                if (!state.history) {
                    return { ...state, history: History.make(action.payload.location.pathname) };
                }
                switch (action.payload.action) {
                    case 'PUSH': {
                        return {
                            ...state,
                            history: History.push(action.payload.location.pathname, state.history),
                        };
                    }
                    case 'POP': {
                    ...
                    default: return exhaustiveCheck(action.payload.action, '');
                }
            }
            return initialState;
        }

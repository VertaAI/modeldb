...
import { withAct, findByDataTestAttribute } from 'core/shared/utils/tests/react/helpers';
...
import { makeBranchesAndTagsQuery, makeCommitWithComponentQuery } from './queries';
...
const makeComponent = async ({
  ...
  const branchesAndTagsQuery = makeBranchesAndTagsQuery(repository.id, { tags, branches: [defaultCommitPointer.value] });
  ...
  return withAct(async () => {
    const data = await makeMountComponentWithPredefinedData({
      ...
      apolloMockedProviderProps: { mocks: [branchesAndTagsQuery, makeCommitWithComponentQuery(repository.id, {
        commitPointer: commitPointer,
        location: location,
        commitWithComponent: {
          commit: commit,
      ...
      })].concat(commitComponents ?
        commitComponents.map(c => (makeCommitWithComponentQuery(repository.id, {
          commitPointer: commitPointer,
          location: c.location,
          commitWithComponent: {
            commit: commit,

...
TEST_FILE="src/test/java/ai/verta/modeldb/blobs/SelfDiff.java"
cat > $TEST_FILE <<EOF
package ai.verta.modeldb.blobs;
...
public class SelfDiff {
    @Property public void SelfDiff(Blob a) throws ModelDBException {
        Blob newA = Utils.sortLists(enforceOneof(a));
...
do
    ...
    cat >> $TEST_FILE <<EOF
    @Property public void SelfDiff${type}(${type}Blob a) throws ModelDBException {
        ${type}Blob newA = Utils.sortLists(enforceOneof(a));
        ${type}Diff d = DiffComputer.compute${type}Diff(newA, newA);
        assertNull(d);
    }

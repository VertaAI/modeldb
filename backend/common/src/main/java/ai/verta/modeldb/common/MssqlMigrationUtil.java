...
public class MssqlMigrationUtil {
  ...
  private static Map<String, Map<String, Set<String>>> getAllTableIndexesForMSSQL(
      Handle handle, Set<String> columnNames) {
    ...
    handle
        .createQuery(
            "SELECT  obj.name AS FK_NAME,\n"
                ...
                + "    ON col2.column_id = referenced_column_id AND col2.object_id = tab2.object_id ") // WHERE col1.name IN (<columnNames>)
        /*.bindList("columnNames", columnNames)*/
        .map(
            (rs, ctx) -> {
              ...
              constraintsMap.put("tableName", new AbstractMap.SimpleEntry<>(tableName, columnName));
              constraintsMap.put(

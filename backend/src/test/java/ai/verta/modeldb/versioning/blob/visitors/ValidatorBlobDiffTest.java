...
public class ValidatorBlobDiffTest {
  ...
  private static final BlobDiff.Builder blobDiffs[] = {
      BlobDiff.newBuilder(),
      BlobDiff.newBuilder().addLocation("location").setConfig(ConfigDiff.newBuilder().addHyperparameters(
          HyperparameterConfigDiff.newBuilder().setB(HyperparameterConfigBlob.newBuilder().setName("test").setValue(
              HyperparameterValuesConfigBlob.newBuilder().setStringValue("test")))).build()),
      BlobDiff.newBuilder().setCode(CodeDiff.newBuilder().setGit(GitCodeDiff.newBuilder().setStatus(
          DiffStatus.ADDED).setB(GitCodeBlob.newBuilder().setRepo("test")))),
      BlobDiff.newBuilder().addLocation("location").setStatus(DiffStatus.MODIFIED).setEnvironment(EnvironmentDiff.newBuilder()
          .addEnvironmentVariables(EnvironmentVariablesDiff.newBuilder().setStatus(DiffStatus.ADDED)
              .setA(EnvironmentVariablesBlob.newBuilder().setName("word1 word2")).setB(EnvironmentVariablesBlob.newBuilder().setName("word1")))),
      BlobDiff.newBuilder().addLocation("location").setStatus(DiffStatus.DELETED).setDataset(
          DatasetDiff.newBuilder().setS3(S3DatasetDiff.newBuilder().addComponents(
              S3DatasetComponentDiff.newBuilder().setPath(PathDatasetComponentDiff.newBuilder().setStatus(DiffStatus.DELETED).setA(
                  PathDatasetComponentBlob.newBuilder().setMd5("test_md5"))))))
  };

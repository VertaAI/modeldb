...
public class CommonCollaboratorResourceMigration {
    private static final Logger LOGGER = LogManager.getLogger(CommonCollaboratorResourceMigration.class);
    ...
    protected  <T extends ResourceEntity> void migrateResources(Supplier<Long> countSupplier, Supplier<Session> sessionSupplier, Class<T> clazz, ModelDBResourceEnum.ModelDBServiceResourceTypes resourceType, HibernateConnection hibernateConnection) {
        ...
        while (lowerBound < count) {
            ...
            try (Session session = sessionSupplier.get()) {
                ...
                if (resourceEntities.size() > 0) {
                    ...
                    List<GetResourcesResponseItem> responseItems =
                            roleService.getResourceItems(
                                    null, newVisibilityresourceIds, resourceType);
                ...
                if (hibernateConnection.needToRetry(ex)) {
                    migrateResources(countSupplier, sessionSupplier, clazz, resourceType, hibernateConnection);
                } else {
                    throw ex;
                }
            }

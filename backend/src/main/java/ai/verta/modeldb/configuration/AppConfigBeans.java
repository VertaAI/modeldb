...
public class AppConfigBeans {
      ...
      switch (artifactStoreConfig.getArtifactStoreType()) {
        case "S3":
          if (!artifactStoreConfig.S3.getS3presignedURLEnabled()) {
            appContext.registeredBean("s3Controller", S3Controller.class);
          }
          artifactStoreService = new S3Service(artifactStoreConfig.S3.getCloudBucketName());
          break;
        case "NFS":
          appContext.registeredBean("nfsController", NFSController.class);
  ...
  private void wrapService(ServerBuilder<?> serverBuilder, BindableService bindableService) {
  ...
  public void onShutDown() {
    if (this.server.isPresent()) {
      ...
      try {
        ...
        var mdbConfig = appContext.applicationContext.getBean(MDBConfig.class);

...
public class ProjectDAORdbImpl implements ProjectDAO {
  ...
  private Project insertProject(Project project, String workspaceName, UserInfo userInfo)
    ...
    try (Session session = modelDBHibernateUtil.getSessionFactory().openSession()) {
      ...
      return projectEntity.getProtoObject(
          roleService, authService, new HashMap<>(), Optional.empty());
    } catch (Exception ex) {
  ...
  public Project updateProjectAttributes(String projectId, KeyValue attribute)
    ...
    try (Session session = modelDBHibernateUtil.getSessionFactory().openSession()) {
      ...
      transaction.commit();
      return projectObj.getProtoObject(roleService, authService, new HashMap<>(), Optional.empty());
    } catch (Exception ex) {
      if (ModelDBUtils.needToRetry(ex)) {
  ...
  public List<KeyValue> getProjectAttributes(
    ...
    try (Session session = modelDBHibernateUtil.getSessionFactory().openSession()) {
      if (getAll) {
        ...
        return projectObj
            .getProtoObject(roleService, authService, new HashMap<>(), Optional.empty())
            .getAttributesList();
      } else {
    ...
    try (Session session = modelDBHibernateUtil.getSessionFactory().openSession()) {
      ...
      Project existingProtoProjectObj =
          projectObj.getProtoObject(
              roleService, authService, cacheWorkspaceMap, Optional.of(getResourcesMap));
  ...
  public List<Artifact> getProjectArtifacts(String projectId)
    ...
    try (Session session = modelDBHibernateUtil.getSessionFactory().openSession()) {
      ...
      Project project =
          projectEntity.getProtoObject(roleService, authService, new HashMap<>(), Optional.empty());

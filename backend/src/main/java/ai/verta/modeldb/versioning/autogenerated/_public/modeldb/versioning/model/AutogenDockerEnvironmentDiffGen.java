...
public class AutogenDockerEnvironmentDiffGen extends Generator<AutogenDockerEnvironmentDiff> {
    ...
    @Override public AutogenDockerEnvironmentDiff generate(
            ...
            GenerationStatus status) {
                ...
                if (r.nextBoolean()) {
                    obj.setA(Utils.removeEmpty(gen().type(AutogenDockerEnvironmentBlob
.class).generate(r, status)));
                }
                if (r.nextBoolean()) {
                    obj.setB(Utils.removeEmpty(gen().type(AutogenDockerEnvironmentBlob
.class).generate(r, status)));
                }
                if (r.nextBoolean()) {
                    obj.setC(Utils.removeEmpty(gen().type(AutogenDockerEnvironmentBlob
.class).generate(r, status)));
                }
                if (r.nextBoolean()) {

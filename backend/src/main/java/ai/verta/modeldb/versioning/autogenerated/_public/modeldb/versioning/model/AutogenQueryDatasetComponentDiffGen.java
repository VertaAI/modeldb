...
public class AutogenQueryDatasetComponentDiffGen
  ...
  @Override
  public AutogenQueryDatasetComponentDiff generate(SourceOfRandomness r, GenerationStatus status) {
    // if (r.nextBoolean())
    //     return null;
    ...
    if (r.nextBoolean()) {
      obj.setA(
          Utils.removeEmpty(
              gen().type(AutogenQueryDatasetComponentBlob.class).generate(r, status)));
    }
    if (r.nextBoolean()) {
      obj.setB(
          Utils.removeEmpty(

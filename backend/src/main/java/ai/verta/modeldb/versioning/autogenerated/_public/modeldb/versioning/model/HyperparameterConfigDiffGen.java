...
public class HyperparameterConfigDiffGen extends Generator<HyperparameterConfigDiff> {
  ...
  public HyperparameterConfigDiff generate(SourceOfRandomness r, GenerationStatus status) {
    ...
    if (r.nextBoolean()) {
      obj.setA(Utils.removeEmpty(gen().type(HyperparameterConfigBlob.class).generate(r, status)));
    }
    if (r.nextBoolean()) {
      obj.setB(Utils.removeEmpty(gen().type(HyperparameterConfigBlob.class).generate(r, status)));
    }
    if (r.nextBoolean()) {
      obj.setStatus(

        ...
        fb.forEach(
            el -> {
              if (el != null) {
                String key = hasher.apply(el);
                if (key != null) {
                  HashSet<F> set = mapB.getOrDefault(key, new HashSet<>());
                  set.add(el);
                  mapB.put(key, set);
                }
              }
        ...
        keys.stream()
            .flatMap(
                key -> {
                  ...
                  //                  if (elA != null && elB != null && elA.size() == 1 &&
                  // elB.size() == 1) {
                  //                    return Stream.of(computer.apply(elA.iterator().next(),
                  // elB.iterator().next()));
                  //                  }

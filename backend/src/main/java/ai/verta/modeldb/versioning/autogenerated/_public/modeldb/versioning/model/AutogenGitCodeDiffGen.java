...
public class AutogenGitCodeDiffGen extends Generator<AutogenGitCodeDiff> {
    ...
    @Override public AutogenGitCodeDiff generate(
            ...
            GenerationStatus status) {
                ...
                if (r.nextBoolean()) {
                    obj.setA(Utils.removeEmpty(gen().type(AutogenGitCodeBlob
.class).generate(r, status)));
                }
                if (r.nextBoolean()) {
                    obj.setB(Utils.removeEmpty(gen().type(AutogenGitCodeBlob
.class).generate(r, status)));
                }
                if (r.nextBoolean()) {
                    obj.setC(Utils.removeEmpty(gen().type(AutogenGitCodeBlob
.class).generate(r, status)));
                }
                if (r.nextBoolean()) {

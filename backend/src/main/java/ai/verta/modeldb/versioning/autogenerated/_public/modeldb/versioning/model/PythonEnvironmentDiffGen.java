...
public class PythonEnvironmentDiffGen extends Generator<PythonEnvironmentDiff> {
    ...
    @Override public PythonEnvironmentDiff generate(
            ...
            GenerationStatus status) {
                ...
                if (r.nextBoolean()) {
                    obj.setVersionStatus(Utils.removeEmpty(gen().type(DiffStatusEnumDiffStatus.class).generate(r, status)));
                }
                if (r.nextBoolean()) {
                    obj.setVersionA(Utils.removeEmpty(gen().type(VersionEnvironmentBlob.class).generate(r, status)));
                }
                if (r.nextBoolean()) {
                    obj.setVersionB(Utils.removeEmpty(gen().type(VersionEnvironmentBlob.class).generate(r, status)));
                }
                if (r.nextBoolean()) {

...
public class AutogenPathDatasetDiffGen extends Generator<AutogenPathDatasetDiff> {
  ...
  @Override
  public AutogenPathDatasetDiff generate(SourceOfRandomness r, GenerationStatus status) {
    // if (r.nextBoolean())
    //     return null;
    ...
    if (r.nextBoolean()) {
      ...
      List<AutogenPathDatasetComponentDiff> ret = new ArrayList(size);
      for (int i = 0; i < size; i++) {
        ret.add(gen().type(AutogenPathDatasetComponentDiff.class).generate(r, status));
      }

...
public class AutogenEnvironmentBlobGen extends Generator<AutogenEnvironmentBlob> {
    ...
    @Override public AutogenEnvironmentBlob generate(
...
> ret = new ArrayList(size);
                ...
                if (r.nextBoolean()) {
                    obj.setDocker(Utils.removeEmpty(gen().type(AutogenDockerEnvironmentBlob
...
> ret = new ArrayList(size);
                    for (int i = 0; i < size; i++) {
                        ret.add(gen().type(AutogenEnvironmentVariablesBlob
.class).generate(r, status));
                    }
                    obj.setEnvironmentVariables(Utils.removeEmpty(ret));
                }
                if (r.nextBoolean()) {
                    obj.setPython(Utils.removeEmpty(gen().type(AutogenPythonEnvironmentBlob
.class).generate(r, status)));
                }
                return obj;
    }
